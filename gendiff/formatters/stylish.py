...
def render_stylish(diff_tree, depth=1):
    ...
    for node in diff_tree:
        ...
        if node_type == 'nested':
            ...
            val1 = stringify(node['old_value'], depth)
            val2 = stringify(node['new_value'], depth)
            lines.append(f"{prefix_indent}- {key}: {val1}")
            lines.append(f"{prefix_indent}+ {key}: {val2}")
